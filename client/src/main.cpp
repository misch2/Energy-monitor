...
void publish_homeassistant_value(String component,            // component type e.g. "sensor", "text", "switch", etc.
  ...
  // ['energymonitor1'], 'name': 'energymonitor1', 'connections': []}, 'enabled_by_default': True, 'entity_category': <EntityCategory.DIAGNOSTIC: 'diagnostic'>,
  // 'device_class': <BinarySensorDeviceClass.LIGHT: 'light'>, 'icon': 'mdi:lightbulb', 'name': 'energymonitor1 backlight', 'unique_id':
  // 'energymonitor1_backlight', 'force_update': False, 'qos': 0, 'availability_mode': 'latest', 'payload_on': 'ON', 'payload_not_available': 'offline',
  // 'encoding': 'utf-8', 'payload_off': 'OFF', 'payload_available': 'online'}, this is not expected. Please correct your configuration. The device name prefix
  // will be stripped off the entity name and becomes 'backlight'
  doc["name"] = key;  // fixed, see above^
  doc["unique_id"] = device_topic + "_" + key;

  String serialized;
  serializeJson(doc, serialized);
  mqttClient.publish(config_topic.c_str(), serialized.c_str(), true);
  mqttClient.publish(state_topic.c_str(), value.c_str(), false);
}

void publish_homeassistant_value_backlight(int on_off) {
  publish_homeassistant_value("binary_sensor", MQTT_HA_DEVICENAME, "backlight", on_off ? "ON" : "OFF", "diagnostic",
...
void publish_homeassistant_value_uptime() {
  ...
  if ((uptime - last_uptime >= 30) || (uptime < last_uptime)) {  // publish every minute or if the value overflowed
    publish_homeassistant_value("sensor", MQTT_HA_DEVICENAME, "uptime", String(uptime), "diagnostic", "duration", "measurement", "s", "mdi:chart-box-outline");
    last_uptime = uptime;
  }
}
...
void setBacklight(int on_off) {  // 0 - 255
  ...
  publish_homeassistant_value_backlight(on_off);
